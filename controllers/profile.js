...
const getProfileInfos = (req, res) => {
    ...
    const getInfos = 'SELECT roles.name AS role, email, avatars.url, DATE_FORMAT( registration_date, "%d/%m/%Y" ) AS registration_date FROM users JOIN roles ON users.role_id = roles.id JOIN avatars ON users.avatar_id = avatars.id WHERE username = ?'
