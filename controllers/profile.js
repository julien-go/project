...
const getProfileInfos = (req, res) => {
    ...
    const getInfos = 'SELECT roles.name, email, avatars.url, registration_date FROM users JOIN roles ON users.role_id = roles.id JOIN avatars ON users.avatar_id = avatars.id WHERE username = ?'
